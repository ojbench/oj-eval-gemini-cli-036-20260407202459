...
struct dynamic_bitset {
    ...
    dynamic_bitset &operator |= (const dynamic_bitset &other) {
        ...
        std::size_t min_blocks = min_len / 64;
        ...
        if (rem > 0) {
            uint64_t mask = (1ULL << rem) - 1;
            blocks[min_blocks] |= (other.blocks[min_blocks] & mask);
        }
        return *this;
    }
    ...
    dynamic_bitset &operator &= (const dynamic_bitset &other) {
        ...
        if (rem > 0) {
            uint64_t mask = (1ULL << rem) - 1;
            blocks[min_blocks] = (blocks[min_blocks] & ~mask) | (blocks[min_blocks] & other.blocks[min_blocks] & mask);
        }
        return *this;
    }
